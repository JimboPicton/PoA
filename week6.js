(function () {
  ...
  const defaults = {tempo:50,stride:64,spacing:78,down:66,up:56,lean:5,knee:72,foot:70,arms:62,overlap:2};
  const presets = {
    natural:{tempo:50,stride:64,spacing:78,down:66,up:56,lean:5,knee:72,foot:70,arms:62,overlap:2},
  ...
  let fps=12,speed=1,frame=0,playing=true,last=performance.now(),compare=false,promptIndex=0;
  const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));
  const mix=(a,b,t)=>a+(b-a)*t;
  const wrap=v=>((v%1)+1)%1;
  const secondsPerStep=()=>+controls.tempo.value/100;
  const totalFrames=()=>Math.max(12,Math.round(secondsPerStep()*2*fps));
  ...
  function footTarget(legPhase,stride,lift,ground,scale,rigid){
    ...
    if(q<.62){
      ...
      else if(t<.58){angle=0;toeAngle=0;state='FLAT SUPPORT'}
      else{const k=smooth((t-.58)/.42);angle=Math.PI/4*k*roll;toeAngle=mix(angle,0,k*roll);state='TOE PUSH-OFF'}
    }else{
      stance=false;t=(q-.62)/.38;x=-stride/2+stride*smooth(t);baseY=ground-Math.sin(Math.PI*t)*lift;
      angle=mix(Math.PI/4,-Math.PI/4,smooth(t))*roll;toeAngle=t<.25?mix(angle,0,(1-t/.25)*roll):angle;state='SWING CLEARANCE';
    }
  ...
  function poseAt(f,rigid=false){const u=easedCycle(f/totalFrames()),w=canvas.clientWidth,h=canvas.clientHeight,scale=Math.min(w,h),ground=h*.87;const stride=scale*(.28+.22*(+controls.stride.value/100));const leg=scale*.225,lower=scale*.225;const pelvis={x:w*.5,y:ground-leg-lower+bodyOffset(u,rigid)};const lift=scale*(.08+.10*(+controls.knee.value/100));const leftFootLocal=footTarget(u,stride,lift,ground,scale,rigid),rightFootLocal=footTarget(u+.5,stride,lift,ground,scale,rigid);const lf={x:pelvis.x+leftFootLocal.x,y:leftFootLocal.y},rf={x:pelvis.x+rightFootLocal.x,y:rightFootLocal.y};const bend=.65+.35*(+controls.knee.value/100);const lk=ik({x:pelvis.x-5,y:pelvis.y},lf,leg,lower,bend),rk=ik({x:pelvis.x+5,y:pelvis.y},rf,leg,lower,bend);const lean=(rigid?0:+controls.lean.value)*Math.PI/180;const torso=scale*.235,shoulder={x:pelvis.x+Math.sin(lean)*torso,y:pelvis.y-Math.cos(lean)*torso};const head={x:shoulder.x+Math.sin(lean)*scale*.09,y:shoulder.y-Math.cos(lean)*scale*.09};const armAmount=(rigid?0.18:+controls.arms.value/100)*.78;const delay=(rigid?0:+controls.overlap.value)/totalFrames();const armWave=Math.sin((u-delay)*Math.PI*2);const upper=scale*.17,fore=scale*.16;function arm(side){const a=Math.PI/2+side*armWave*armAmount;const elbow={x:shoulder.x+Math.cos(a)*upper,y:shoulder.y+Math.sin(a)*upper};const lagWave=Math.sin((u-delay*1.8)*Math.PI*2);const b=a+side*lagWave*(rigid?.04:.22);const hand={x:elbow.x+Math.cos(b)*fore,y:elbow.y+Math.sin(b)*fore};return{elbow,hand}}return{u,ground,scale,pelvis,shoulder,head,lf,rf,lk,rk,leftArm:arm(1),rightArm:arm(-1),leftFootLocal,rightFootLocal,lean}}
  ...
  function drawFigure(p,alpha=1,rigid=false){const s=p.scale,outline=rigid?'#68736f':'#173b39',fill=rigid?'#dfe4e2':'#ffd3c4',bone=s*.035;line(p.pelvis,p.shoulder,bone+s*.012,outline,alpha);line(p.pelvis,p.shoulder,bone,fill,alpha);for(const [hip,knee,foot,info] of [[{x:p.pelvis.x-5,y:p.pelvis.y},p.lk,p.lf,p.leftFootLocal],[{x:p.pelvis.x+5,y:p.pelvis.y},p.rk,p.rf,p.rightFootLocal]]){line(hip,knee,bone*.86+s*.008,outline,alpha);line(knee,foot,bone*.72+s*.008,outline,alpha);line(hip,knee,bone*.86,fill,alpha);line(knee,foot,bone*.72,fill,alpha);joint(knee,s*.018,'#fff8f3',outline,alpha);footShape(foot,info,s,fill,outline,alpha)}for(const arm of [p.leftArm,p.rightArm]){line(p.shoulder,arm.elbow,bone*.6+s*.006,outline,alpha);line(arm.elbow,arm.hand,bone*.48+s*.006,outline,alpha);line(p.shoulder,arm.elbow,bone*.6,fill,alpha);line(arm.elbow,arm.hand,bone*.48,fill,alpha);joint(arm.elbow,s*.014,'#fff8f3',outline,alpha);joint(arm.hand,s*.012,fill,outline,alpha)}joint(p.pelvis,s*.026,fill,outline,alpha);joint(p.shoulder,s*.03,fill,outline,alpha);joint(p.head,s*.052,fill,outline,alpha);return p}
  function drawPaths(){
    const w=canvas.clientWidth,h=canvas.clientHeight,left=w*.08,right=w*.92;
    ctx.save();ctx.lineWidth=2;ctx.setLineDash([6,7]);
    for(const [point,colour] of [['head','rgba(239,91,63,.32)'],['pelvis','rgba(29,110,104,.42)']]){ctx.strokeStyle=colour;ctx.beginPath();for(let i=0;i<=96;i++){const p=poseAt(i/96*totalFrames()*.5),pt=p[point],x=mix(left,right,i/96);if(i)ctx.lineTo(x,pt.y);else ctx.moveTo(x,pt.y)}ctx.stroke()}
  ...
  function drawPositionStrip(){const phases=[0,.125,.25,.375,.5],names=['CONTACT','DOWN','PASSING','UP','CONTACT'];const w=canvas.clientWidth,h=canvas.clientHeight,active=Math.round(wrap(frame/totalFrames()*2)*4)%4;ctx.save();ctx.translate(0,h*.02);for(let i=0;i<phases.length;i++){const p=poseAt(phases[i]*totalFrames());const x=w*(.11+i*.195),factor=.54,base=h*.87,isActive=(i%4)===active;ctx.save();ctx.translate(x-p.pelvis.x*factor,base-p.ground*factor);ctx.scale(factor,factor);drawFigure(p,isActive?1:.34,false);ctx.restore();label(names[i],{x,y:base+18},isActive?'#ef5b3f':'#66716e')}ctx.restore()}
  ...
  function drawScene(){const w=canvas.clientWidth,h=canvas.clientHeight,p=poseAt(frame);ctx.clearRect(0,0,w,h);ctx.fillStyle='#e7f0ed';ctx.fillRect(0,0,w,h*.88);ctx.fillStyle='#ece3d2';ctx.fillRect(0,h*.88,w,h*.12);ctx.strokeStyle='#b8ad92';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,h*.87);ctx.lineTo(w,h*.87);ctx.stroke();if($('#pathToggle').checked)drawPaths();if($('#posesToggle').checked)drawPositionStrip();else{if(compare)drawFigure(poseAt(frame,true),.3,true);const g=drawFigure(p);if($('#explainToggle').checked){label('PELVIS · WEIGHT PATH',{x:g.pelvis.x,y:g.pelvis.y-30},'#ef5b3f');const support=[g.leftFootLocal,g.rightFootLocal].find(foot=>foot.stance)||g.leftFootLocal;const supportPoint=support===g.leftFootLocal?g.lf:g.rf;label(support.state,{x:supportPoint.x,y:supportPoint.y+34},'#1d6e68');label('OPPOSING ARM SWING',{x:g.leftArm.hand.x,y:g.leftArm.hand.y-24},'#1d6e68')}}const info=phaseInfo(p.u);$('#phaseTitle').textContent=info[0];$('#phaseNote').textContent=info[1];$('#motionLabel').textContent=info[2]}
  function drawMap(){const w=map.clientWidth,h=map.clientHeight,left=70,right=w-16,total=totalFrames(),half=total/2;const beats=[['CONTACT',0,'#ef5b3f'],['DOWN',half*.25,'#e1a22b'],['PASS',half*.5,'#1d6e68'],['UP',half*.75,'#1d6e68'],['CONTACT',half,'#ef5b3f'],['DOWN',half*1.25,'#e1a22b'],['PASS',half*1.5,'#1d6e68'],['UP',half*1.75,'#1d6e68'],['CONTACT',total-1,'#ef5b3f']];mctx.clearRect(0,0,w,h);mctx.strokeStyle='#d9d9d1';mctx.beginPath();mctx.moveTo(left,h*.54);mctx.lineTo(right,h*.54);mctx.stroke();mctx.font='700 8px Inter,Arial,sans-serif';for(const [name,f,col] of beats){const x=left+(right-left)*f/(total-1);mctx.fillStyle=col;mctx.beginPath();mctx.arc(x,h*.54,5,0,Math.PI*2);mctx.fill();mctx.fillStyle='#5e6966';mctx.textAlign='center';mctx.fillText(name,x,h*.31);mctx.fillText(Math.round(f)+1,x,h*.77)}mctx.fillStyle='#66716e';mctx.textAlign='left';mctx.fillText('BODY / FEET',4,h*.57)}
  ...
  function reset(){Object.entries(defaults).forEach(([k,v])=>controls[k].value=v);fps=12;speed=1;frame=0;playing=true;compare=false;$('#pathToggle').checked=true;$('#posesToggle').checked=false;$('#explainToggle').checked=false;$('#playBtn').textContent='Ⅱ';$('#compareBtn').textContent='Compare rigid walk';setSpeed(1);setFps(12);applyPreset('natural')}
  ...
  Object.values(controls).forEach(c=>c.addEventListener('input',sync));$$('[data-fps]').forEach(b=>b.addEventListener('click',()=>setFps(+b.dataset.fps)));$$('[data-speed]').forEach(b=>b.addEventListener('click',()=>setSpeed(+b.dataset.speed)));$$('[data-preset]').forEach(b=>b.addEventListener('click',()=>applyPreset(b.dataset.preset)));
  ...
  $('#compareBtn').addEventListener('click',()=>{compare=!compare;$('#compareBtn').textContent=compare?'Hide rigid walk':'Compare rigid walk';draw()});$('#fullscreenBtn').addEventListener('click',()=>$('#canvasWrap').requestFullscreen?.());['pathToggle','posesToggle'].forEach(id=>$('#'+id).addEventListener('change',draw));$('#explainToggle').addEventListener('change',()=>{if($('#explainToggle').checked)setSpeed(.25);draw()});$('#resetAll').addEventListener('click',reset);$('#resetControls').addEventListener('click',reset);$('#newPrompt').addEventListener('click',()=>{promptIndex=(promptIndex+1)%prompts.length;$('#promptText').textContent=prompts[promptIndex]});
