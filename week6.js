(function () {
  ...
  function drawFigure(p,alpha=1,rigid=false){const s=p.scale,outline=rigid?'#68736f':'#173b39',fill=rigid?'#dfe4e2':'#ffd3c4',bone=s*.035;function drawArm(arm,armAlpha=alpha){line(p.shoulder,arm.elbow,bone*.6+s*.006,outline,armAlpha);line(arm.elbow,arm.hand,bone*.48+s*.006,outline,armAlpha);line(p.shoulder,arm.elbow,bone*.6,fill,armAlpha);line(arm.elbow,arm.hand,bone*.48,fill,armAlpha);joint(arm.elbow,s*.014,'#fff8f3',outline,armAlpha);joint(arm.hand,s*.012,fill,outline,armAlpha)}drawArm(p.leftArm,alpha*.68);line(p.pelvis,p.shoulder,bone+s*.012,outline,alpha);line(p.pelvis,p.shoulder,bone,fill,alpha);for(const [hip,knee,foot,info] of [[{x:p.pelvis.x-5,y:p.pelvis.y},p.lk,p.lf,p.leftFootLocal],[{x:p.pelvis.x+5,y:p.pelvis.y},p.rk,p.rf,p.rightFootLocal]]){line(hip,knee,bone*.86+s*.008,outline,alpha);line(knee,foot,bone*.72+s*.008,outline,alpha);line(hip,knee,bone*.86,fill,alpha);line(knee,foot,bone*.72,fill,alpha);joint(knee,s*.018,'#fff8f3',outline,alpha);footShape(foot,info,s,fill,outline,alpha)}joint(p.pelvis,s*.026,fill,outline,alpha);joint(p.shoulder,s*.03,fill,outline,alpha);joint(p.head,s*.052,fill,outline,alpha);drawArm(p.rightArm);return p}
  function drawPaths(){
